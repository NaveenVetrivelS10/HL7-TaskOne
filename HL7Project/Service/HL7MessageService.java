...
public class HL7MessageService implements HL7MessageHandlerItf {
	...
	public String checkIDExist(HL7MessageRequest request) throws Exception {
		Optional<MessageHeader> checkMessageId = messageHeaderRepo.findById(request.getId());
		// Optional<MessageHeader> checkMessageId =
		// messageHeaderRepo.findByMid(request.getId());
		...
		if (!isExist) {
			...
			sender.setIdMessageHeader(request.getId());
			...
			receiver.setIdMessageHeader(request.getId());
			...
			acknowledgements.setIdMessageHeader(request.getId());
			...
			Optional<MessageHeader> optionalMessageHeader = messageHeaderRepo.findById(request.getId());
			if(optionalMessageHeader.isEmpty()) {
				throw new EntityNotFoundException("Invalid id, data is not exist is Message Header table");
			} 
			MessageHeader msgHeader = optionalMessageHeader.get();
			msgHeader.setFieldSeparator(messageValues.get("Field Separator"));
			msgHeader.setEncodingCharacters(messageValues.get("Encoding Characters"));
			msgHeader.setMessageType(messageValues.get("Message Type"));
			msgHeader.setMessageControlId(messageValues.get("Message Control ID"));
			...
			msgHeader.setVersionId(messageValues.get("Version ID"));
			msgHeader.setDateTimeOfMessage(messageValues.get("DateTime Of Message"));
			msgHeader = messageHeaderRepo.save(msgHeader); // }
			
			// Update the Sender object
			Optional<Sender> optionalSender = senderRepo.findById(request.getId());
			...
			Optional<Receiver> optionalReceiver = receiverRepo.findById(request.getId());
			...
			// Update the Acknowledgements object
			Optional<Acknowledgements> optionalAcknowledgements = acknowledgementsRepo.findById(request.getId());
			// if(optionalAcknowledgements.isPresent()) // {
			Acknowledgements acknowledgements = optionalAcknowledgements.get();
			acknowledgements.setAcceptAcknowledgmentType(messageValues.get("Accept Acknowledgment Type"));
			acknowledgements.setApplicationAcknowledgmentType(messageValues.get("Application Acknowledgment Type"));
			acknowledgements.setCountryCode(messageValues.get("Country Code"));
			acknowledgementsRepo.save(acknowledgements);// }
			return "update scussfull";
